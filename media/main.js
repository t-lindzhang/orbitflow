...
(() => {
  ...
  var priorityIds = /* @__PURE__ */ new Set();
  var svg = select_default2("#graph");
  var card = document.getElementById("card");
  var empty2 = document.getElementById("empty");
  var currentTransform = identity2;
  var userInteracted = false;
  var lastBounds = null;
  var zoomBehavior = zoom_default2().scaleExtent([0.2, 3]).filter((event) => {
    if (event.type === "wheel") {
  ...
  function fitToView(b) {
    ...
    const t = identity2.translate(sw / 2 - scale * cx, sh / 2 - scale * cy).scale(scale);
  ...
  function resetZoom() {
    ...
    if (lastBounds) {
      ...
      zoomBehavior.transform(svg, identity2);
    }
  }
  document.getElementById("btn-revert").addEventListener("click", () => vscode.postMessage({ type: "revert" }));
  document.getElementById("btn-generate").addEventListener(
    "click",
    () => vscode.postMessage({ type: "generateTrees" })
  );
  document.getElementById("btn-open")?.addEventListener("click", () => vscode.postMessage({ type: "openGraph" }));
  document.getElementById("btn-reorganize").addEventListener("click", () => vscode.postMessage({ type: "reorganize" }));
  document.getElementById("btn-clear").addEventListener("click", () => vscode.postMessage({ type: "clearAll" }));
  ...
  function treeColor(treeId) {
    return state.trees.find((t) => t.id === treeId)?.baseColor ?? "#888";
  }
  function nodeFillRgb(hex2, relevance, xNorm) {
    const hsl2 = rgbToHsl(hexToRgb(hex2));
    hsl2.h = (hsl2.h + xNorm * HUE_SHIFT + 360) % 360;
    const shifted = hslToRgb(hsl2);
    const gray = 128;
    const t = Math.max(0, Math.min(1, relevance));
    const mix = (v) => Math.round(gray + (v - gray) * t);
  ...
  function computeLayout(nodes) {
    ...
    for (const n of nodes) {
      ...
      (children2.get(key) ?? children2.set(key, []).get(key)).push(n);
    }
    const pos = /* @__PURE__ */ new Map();
    let cursor = 0;
    const place = (node, depth) => {
      const kids = children2.get(node.id) ?? [];
      let x;
      if (kids.length === 0) {
        x = cursor * LEAF_GAP;
        cursor++;
    ...
    for (const root2 of children2.get("__root__") ?? []) {
      place(root2, 0);
      cursor++;
    }
    return pos;
  ...
  function computeDepths(nodes) {
    const byId = new Map(nodes.map((n) => [n.id, n]));
    const depths2 = /* @__PURE__ */ new Map();
    for (const n of nodes) {
      let depth = 0;
      let cur = n;
      ...
      depths2.set(n.id, depth);
    }
    return depths2;
  }
  var depths = /* @__PURE__ */ new Map();
  function render() {
    empty2.classList.toggle("hidden", state.nodes.length > 0);
    ...
    const norm = (r) => maxRel > minRel ? (r - minRel) / (maxRel - minRel) : 1;
    let minX = Infinity;
    let minY = Infinity;
    let maxX = 0;
    let maxY = 0;
    ...
    const xNorm = (x) => spanX > 0 ? Math.max(-1, Math.min(1, (x - midX) / spanX)) : 0;
    svg.selectAll("*").remove();
    const viewport = svg.append("g").attr("class", "viewport").attr("transform", currentTransform.toString());
    const fillById = /* @__PURE__ */ new Map();
    for (const n of nodes) {
    ...
    const edgeSel = viewport.append("g").attr("class", "links").selectAll("g.edge").data(links, (d) => d.child).join("g").attr("class", "edge");
    edgeSel.append("path").attr("class", "link").attr("stroke", (d) => d.color).attr("d", pathOf);
    edgeSel.append("path").attr("class", "edge-hit").attr("d", pathOf);
    const prune = edgeSel.append("g").attr("class", "prune-btn").attr(
      "transform",
      (d) => `translate(${(d.from.x + d.to.x) / 2}, ${(d.from.y + d.to.y) / 2})`
    );
    prune.append("circle").attr("r", 9);
    prune.append("text").attr("text-anchor", "middle").attr("dy", "0.35em").text("\u2212");
    edgeSel.on("mouseenter", function() {
      select_default2(this).classed("hover", true);
    }).on("mouseleave", function() {
      select_default2(this).classed("hover", false);
    });
    prune.on("click", (event, d) => {
      event.stopPropagation();
      vscode.postMessage({ type: "pruneSubtree", nodeId: d.child });
    });
    const nodeSel = viewport.append("g").attr("class", "nodes").selectAll("g").data(nodes, (d) => d.id).join("g").attr("class", "node").attr("transform", (d) => {
    ...
    nodeSel.each(function(d) {
      ...
      if (d.type === "session") {
        g.append("rect").attr("x", -r).attr("y", -r).attr("width", r * 2).attr("height", r * 2).attr("rx", 2).attr("fill", fill);
      } else if (d.type === "idea") {
        g.append("polygon").attr("points", `0,${-r - 2} ${r + 1},${r} ${-r - 1},${r}`).attr("fill", fill);
      } else {
      ...
      if (priorityIds.has(d.id)) {
        ...
        g.append("circle").attr("r", r + 4).attr("class", "priority-ring");
      }
      if (d.urgent) {
        g.append("circle").attr("r", r + 5).attr("class", "urgent-ring");
      }
      if (d.status === "done") {
        g.classed("done", true);
        const check = g.append("g").attr("class", "check").attr("transform", `translate(${r - 1}, ${-r + 1})`);
        check.append("circle").attr("r", 6);
        check.append("text").attr("text-anchor", "middle").attr("dy", "0.34em").text("\u2713");
      }
      g.append("text").attr("class", "depth-num").attr("x", 0).attr("y", 0).attr("dy", "0.35em").attr("text-anchor", "middle").text(depths.get(d.id) ?? 0);
      const lines = wrapLabel(d.title, 16, 2);
      const label = g.append("text").attr("class", "label").attr("x", 0).attr("y", r + 14).attr("text-anchor", "middle");
      lines.forEach((ln, i) => {
        label.append("tspan").attr("x", 0).attr("dy", i === 0 ? 0 : 12).text(ln);
      });
      g.append("text").attr("class", "sublabel").attr("x", 0).attr("y", r + 16 + lines.length * 12).attr("text-anchor", "middle").text(timeAgo(d.lastActiveAt));
    });
  ...
  function showCard(node, sticky) {
    const files = node.snapshot.files.map((f) => `<li>${escapeHtml(shortPath(f.path))}${f.line !== void 0 ? `:${f.line + 1}` : ""}</li>`).join("");
    card.innerHTML = `
    <div class="card-head">
      <span class="card-title">${escapeHtml(node.title)}</span>
      ${sticky ? '<span class="card-close" id="card-close">\xD7</span>' : ""}
    </div>
    <div class="card-meta">${node.type} \xB7 ${node.status === "done" ? "done" : "open"} \xB7 ${timeAgo(node.lastActiveAt)}</div>
    <div class="card-detail">${escapeHtml(node.detail)}</div>
    ${files ? `<ul class="card-files">${files}</ul>` : ""}
    ${sticky ? `<div class="card-actions">
            <button id="card-done">${node.status === "done" ? "Reopen" : "Mark done"}</button>
  ...
  `;
    ...
    if (sticky) {
      document.getElementById("card-done")?.addEventListener(
        "click",
        () => vscode.postMessage({ type: "toggleDone", nodeId: node.id })
      );
      document.getElementById("card-resume")?.addEventListener(
        "click",
